...
class RandomDigitsTest < Minitest::Test
  ...
  def test_generate_creates_correct_size
    assert_equal 1, random_gen(1).length
    assert_equal 2, random_gen(2).length
    assert_equal 3, random_gen(3).length
    assert_equal 4, random_gen(4).length
    assert_equal 5, random_gen(5).length
  end

  def test_generate_creates_strings_of_numbers
    nums = []
    10.times { nums << random_gen(5) }
